...
public class DirectoryObserver: NSObject{
    ...
    init(watchedUrl: URL){
        ...
        let contents = (try? FileManager.default.contentsOfDirectory(at: watchedUrl, includingPropertiesForKeys: [.isDirectoryKey], options: .skipsHiddenFiles)) ?? []
        self.previousContents = Set(contents)
    }
    public class func watch(_ url: URL) -> DirectoryObserver?{
        let directoryWatcher = DirectoryObserver(watchedUrl: url)
...
extension DirectoryObserver{
    ...
    private func pollDirectoryForChangesWith(_ oldMetadata: [String]){
        ...
        if self.directoryChanging || 0 < self.retriesLeft {
            ...
            self.checkChanges(after: DirectoryObserver.pollInterval)
        } else {
   ...
   private func directoryDidChange() {
       ...
       self.retriesLeft = DirectoryObserver.retryCount
