...
import AppsFlyerLib
class MoreSubsTableViewController: UITableViewController {
    ...
    func getProductInfo(){
        trial.text = NSLocalizedString("trialYear", comment: "trial")
            ...
            if let product = result.retrievedProducts.first {
                ...
                if hasSubed == true {
                    DispatchQueue.main.async {
                        let week = NSLocalizedString("year",comment: "year")
        ...
        SwiftyStoreKit.retrieveProductsInfo([""]) {
            ...
            if let product = result.retrievedProducts.first {
                ...
                DispatchQueue.main.async {
                    self.oneYearSub.setTitle(priceString + year, for: .normal)
                }
                self.oneYearPriceString = priceString + year
                print("Product: \(product.localizedDescription), price: \(priceString)")
            }
            else if let invalidProductId = result.invalidProductIDs.first {
                print("Invalid product identifier: \(invalidProductId)")
    ...
    @IBAction func purchaseOneMonth(_ sender: Any) {
        ...
        SwiftyStoreKit.purchaseProduct("", quantity: 1, atomically: true) {
            result in
            switch result {
    ...
    func startActivity(sender: UIButton){
        if sender.tag == 0{
            ...
            activityIndicator.center.y = self.oneYearSub.center.y
            self.oneYearSub.setTitle("", for: .normal)
            self.activityIndicator.startAnimating()
        }
    }
    ...
    func stopActivity(){
        ...
        self.oneYearSub.setTitle(oneYearPriceString, for: .normal)
        self.activityIndicator.stopAnimating()
    }
