...
class DocumentsViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, ImageScannerControllerDelegate, UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        directoryObserver.onDeletedFiles = {
            d in
            DispatchQueue.main.async {
                self.documents = Documents.getDocuments()
                self.collection.reloadData()
    ...
    @IBAction func Sort(_ sender: UIBarButtonItem) {
        ...
        actionsheet.addAction(UIAlertAction(title: NSLocalizedString("date", comment: "Date"), style: .default, handler: {
            (action) in
        ...
        actionsheet.addAction(UIAlertAction(title: NSLocalizedString("pageCount", comment: "Page count"), style: .default, handler: {
            (action) in
            if sender.tag % 2 == 0{
                self.documents = self.documents.sorted(by: {
                    $1.pdf.pageCount < $0.pdf.pageCount
                })
                self.collection.reloadData()
                self.tableView.reloadData()
            }
        ...
        actionsheet.addAction(UIAlertAction(title: NSLocalizedString("name", comment: "name"), style: .default, handler: {
            (alert) in
            if sender.tag % 2 == 0{
                self.documents = self.documents.sorted(by:
                                                       {
                ...
                self.documents = self.documents.sorted(by: {
                    $1.pdf.documentURL!.lastPathComponent.lowercased() < $0.pdf.documentURL!.lastPathComponent.lowercased()
                })
                self.collection.reloadData()
                self.tableView.reloadData()
            }
    ...
    func showActionsheet(){
        ...
        actionsheet.addAction(UIAlertAction(title: NSLocalizedString("camera", comment: "camera"), style: .default, handler: {
            (action) in
            self.scanImage()
        }))
        actionsheet.addAction(UIAlertAction(title: NSLocalizedString("photos", comment: "photos"), style: .default, handler: {
            (action) in
            self.selectImage()
        }))
        actionsheet.addAction(UIAlertAction(title: CANCEL, style: .cancel, handler: nil))
